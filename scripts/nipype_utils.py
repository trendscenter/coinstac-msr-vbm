...
def calculate_mask(args):
    ...
    # Threshold binarizer
    """pass this values to inputspec file for configurable options"""
    ...
    try:
        mask_mni = nib.load(mni_image)
        log(f'path: {mni_image} mask_mni_shape: {mask_mni.shape}\n clipped image: {clipped_img.shape}', state_)
    except:
        log(f'failed to load: {mni_image} & clipped_image: {clipped_img.shape}', state_)
